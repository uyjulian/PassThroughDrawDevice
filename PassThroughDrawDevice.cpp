...
class tTVPDrawer_GDI : public tTVPDrawer
{
	...
	typedef tTVPDrawer_GDI selfclass;
protected:
	HDC TargetDC;
...
class tTVPDrawer_DrawDibNoBuffering : public tTVPDrawer_GDI
{
	...
	typedef tTVPDrawer_DrawDibNoBuffering selfclass;
...
{
	typedef tTVPDrawer_GDI inherited;
	typedef tTVPDrawer_GDIDoubleBuffering selfclass;
...
public:
	...
	void CreateBitmap()
	{
		...
		{
			struct CreateBitmapTryCatchBlock {
				selfclass * Self;
				static void TJS_USERENTRY Try(void * data) {
					CreateBitmapTryCatchBlock *param = reinterpret_cast<CreateBitmapTryCatchBlock *>(data);
					HDC screendc = GetDC(param->Self->TargetWindow);
					if(!screendc) TVPThrowExceptionMessage(TJS_W("Failed to create screen DC"));
					param->Self->OffScreenBitmap = CreateCompatibleBitmap(screendc, param->Self->SrcWidth, param->Self->SrcHeight);
					if(!param->Self->OffScreenBitmap) TVPThrowExceptionMessage(TJS_W("Failed to create offscreen bitmap"));
				...
				static bool TJS_USERENTRY Catch(void * data, const tTVPExceptionDesc & desc) {
					CreateBitmapTryCatchBlock *param = reinterpret_cast<CreateBitmapTryCatchBlock *>(data);
					param->Self->DestroyBitmap();
					return true; // yes rethrow
				}
			} param = { this };
			TVPDoTryBlock(&CreateBitmapTryCatchBlock::Try, &CreateBitmapTryCatchBlock::Catch, NULL, (void*)&param);
		}
	}
...
{
	typedef tTVPDrawer inherited;
	typedef tTVPDrawer_DDDoubleBuffering selfclass;
...
public:
	...
	bool NotifyLayerResize(tjs_int w, tjs_int h)
	{
		...
		{
			bool success = true;
			struct NotifyLayerResizeTryCatchBlock {
				selfclass * Self;
				bool * Success;
				static void TJS_USERENTRY Try(void * data) {
					NotifyLayerResizeTryCatchBlock *param = reinterpret_cast<NotifyLayerResizeTryCatchBlock *>(data);

					param->Self->CreateOffScreenSurface();
				}
				static bool TJS_USERENTRY Catch(void * data, const tTVPExceptionDesc & desc) {
					NotifyLayerResizeTryCatchBlock *param = reinterpret_cast<NotifyLayerResizeTryCatchBlock *>(data);

					if (desc.type == TJS_W("eTJS")) {
						TVPAddImportantLog( TJS_W("Passthrough: Failed to create DirectDraw off-screen buffer: ") + desc.message );
					} else {
			...
			TVPDoTryBlock(&NotifyLayerResizeTryCatchBlock::Try, &NotifyLayerResizeTryCatchBlock::Catch, NULL, (void*)&param);
			return success;
		}
		return false;
	}
...
{
	typedef tTVPDrawer inherited;
	typedef tTVPDrawer_D3DDoubleBuffering selfclass;
...
public:
	...
	bool SetDestSize(tjs_int width, tjs_int height)
	{
		...
		{
			bool success = true;
			struct SetDestSizeTryCatchBlock {
				selfclass * Self;
				bool * Success;
				static void TJS_USERENTRY Try(void * data) {
					SetDestSizeTryCatchBlock *param = reinterpret_cast<SetDestSizeTryCatchBlock *>(data);
				...
				static bool TJS_USERENTRY Catch(void * data, const tTVPExceptionDesc & desc) {
					...
					if (desc.type == TJS_W("eTJS")) {
						TVPAddImportantLog( TJS_W("Passthrough: Failed to create Direct3D devices: ") + desc.message );
					} else {
			...
			TVPDoTryBlock(&SetDestSizeTryCatchBlock::Try, &SetDestSizeTryCatchBlock::Catch, NULL, (void*)&param);
			return success;
		}
		return false;
	}
	...
	bool SetDestSizeAndNotifyLayerResize(tjs_int width, tjs_int height, tjs_int w, tjs_int h)
	{
		...
		{
			bool success = true;
			struct SetDestSizeAndNotifyLayerResizeTryCatchBlock {
				selfclass * Self;
				bool * Success;
				static void TJS_USERENTRY Try(void * data) {
					SetDestSizeAndNotifyLayerResizeTryCatchBlock *param = reinterpret_cast<SetDestSizeAndNotifyLayerResizeTryCatchBlock *>(data);
			...
			TVPDoTryBlock(&SetDestSizeAndNotifyLayerResizeTryCatchBlock::Try, &SetDestSizeAndNotifyLayerResizeTryCatchBlock::Catch, NULL, (void*)&param);
			return success;
		}
		return false;
	}
...
void tTVPPassThroughDrawDevice::CreateDrawer(tDrawerType type)
{
	...
	struct SetDestSizeAndNotifyLayerResizeTryCatchBlock {
		...
		static void TJS_USERENTRY Try(void * data) {
			...
			if(param->Self->Drawer)
				param->Self->Drawer->SetTargetWindow(param->Self->TargetWindow);

			if(param->Self->Drawer)
			{
				if(!param->Self->Drawer->SetDestPos(param->Self->DestRect.left, param->Self->DestRect.top))
				{
					TVPAddImportantLog(
						TJS_W("Passthrough: Failed to set destination position to draw device drawer"));
					delete param->Self->Drawer, param->Self->Drawer = NULL;
				}
			}
			...
			{
				tjs_int srcw, srch;
				param->Self->GetSrcSize(srcw, srch);
				if(!param->Self->Drawer->SetDestSizeAndNotifyLayerResize(param->Self->DestRect.get_width(), param->Self->DestRect.get_height(), srcw, srch))
				{
					TVPAddImportantLog(
			...
			if(param->Self->Drawer) param->Self->DrawerType = *(param->Type); else param->Self->DrawerType = dtNone;

			param->Self->RequestInvalidation(tTVPRect(0, 0, param->Self->DestRect.get_width(), param->Self->DestRect.get_height()));
		}
		static bool TJS_USERENTRY Catch(void * data, const tTVPExceptionDesc & desc) {
			SetDestSizeAndNotifyLayerResizeTryCatchBlock *param = reinterpret_cast<SetDestSizeAndNotifyLayerResizeTryCatchBlock *>(data);

			if (desc.type == TJS_W("eTJS")) {
				TVPAddImportantLog( TJS_W("Passthrough: Failed to create drawer: ") + desc.message );
			} else {
				TVPAddImportantLog( TJS_W("Passthrough: Failed to create drawer: unknown reason") );
			}
			param->Self->DestroyDrawer();
			return false; // no rethrow
		}
	} param = { this, &type };
	TVPDoTryBlock(&SetDestSizeAndNotifyLayerResizeTryCatchBlock::Try, &SetDestSizeAndNotifyLayerResizeTryCatchBlock::Catch, NULL, (void*)&param);
		...
		for(int i = 0; i < num_types; i++)
		{
			...
			struct SetDestRectangleTryCatchBlock {
				...
				static void TJS_USERENTRY Try(void * data) {
					...
					// drawer を作成
					param->Self->CreateDrawer(param->type);
					if(!param->Self->Drawer)
					{
						TVPAddImportantLog(TJS_W("Passthrough: Could not create drawer object ") + ttstr(param->type_name));
						param->should_continue = true;
						return;
					}

					// ズーム補間の設定は受け入れられるか？
					int caps = param->Self->Drawer->GetInterpolationCapability();
					if(TVPZoomInterpolation && !(caps & 2))
					{
						TVPAddImportantLog(TJS_W("Passthrough: Drawer object ") + ttstr(param->type_name) +
					...
					param->Self->Drawer->InitTimings();
					static const DWORD timeout = 333;
					DWORD start_tick = timeGetTime();
					int count = 0;
					while(timeGetTime() - start_tick < timeout)
					{
						param->Self->Drawer->StartBitmapCompletion();
						param->Self->Drawer->NotifyBitmapCompleted(0, 0, param->memblk, (const BITMAPINFO *)(param->bmi), *(param->cliprect));
						param->Self->Drawer->EndBitmapCompletion();
						param->Self->Drawer->Show();
						count ++;
					}
					DWORD end_tick = timeGetTime();
					param->Self->Drawer->ReportTimings();

					// 結果を格納、それとデバッグ用に表示
					param->score = count * 1000 / (float)(end_tick - start_tick);
					char msg[80];
					sprintf(msg, "%.2f fps", (float)param->score);
					TVPAddImportantLog(TJS_W("Passthrough: benchmark result: ") + ttstr(param->type_name) + TJS_W(" : ") +
						msg);

					// GDI は最後の手段
					// 結果だけは計っておくが、これが候補になるのは
					// ほかのdrawerに失敗したときのみ
					if(param->type == dtDBGDI)
						param->score = 0.0f;

					// DirectDraw + Vista チェック
					// これも結果だけは測っておくが、これが候補になるのは
					// ほかの drawer に失敗したときのみ
					if(param->type == dtDBDD)
					{
			...
			} param = { this, results[i].score, results[i].type, type_names[i], memblk, &bmi, &cliprect, false };
			...
			results[i].type = param.type;
			results[i].score = param.score;
			DestroyDrawer();
		}
...
void tTVPPassThroughDrawDevice::OnManagerSetError(tjs_error r)
{
	switch (r) {
	case TJS_E_ACCESSDENYED:   TVPThrowExceptionMessage( TJS_W("\"passthrough\" device does not support layer manager more than 1") ); break;
	case TJS_E_MEMBERNOTFOUND: TVPThrowExceptionMessage( TJS_W("\"passthrough\" device removed unknown layer manager") ); break;
	}
}
//---------------------------------------------------------------------------
...
#else
#define TJS_NATIVE_CLASSID_NAME tTJSNC_PassThroughDrawDevice::ClassID
tTJSNativeClassForPlugin* tTJSNC_PassThroughDrawDevice::CreateNativeClass()
#endif
{
#if 1
	tTJSNativeClassForPlugin * classobj =
		TJSCreateNativeClassForPlugin(TJS_W("PassThroughDrawDevice"), &tTJSNC_PassThroughDrawDevice::CreateNativeInstance);
#endif
...
{
	Device = new tTVPPassThroughDrawDeviceWrapper();
}
//---------------------------------------------------------------------------
tTJSNI_PassThroughDrawDevice::~tTJSNI_PassThroughDrawDevice()
{
